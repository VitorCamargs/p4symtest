#!/usr/bin/env python3
"""
Orquestrador de Teste Exaustivo do P4SymTest

Este script:
1. Gera programas P4 sintéticos (com lógica de ingress paralela)
2. Compila cada programa
3. Executa o parser para obter estados iniciais
4. Explora TODOS os caminhos possíveis através das condicionais (EM PARALELO)
5. Executa tabelas de acordo com os caminhos encontrados
6. Executa o deparser
7. Coleta métricas detalhadas (tempo, memória, estados)
8. Gera relatórios e gráficos
"""
...
class PipelinePathExplorer:
    ...
    def _explore_node(self, node_name: str, tables: List[str], 
        ...
        if table:
            # É uma tabela - adiciona e continua pelo next padrão
...
# --- Executor (COM CORREÇÃO DE DEADLOCK) ---

# --- Executor (COM CORREÇÃO DE DEADLOCK v2) ---
...
class ExhaustivePipelineExecutor:
    ...
    def compile_p4(self, p4_file: Path, output_dir: Path) -> Tuple[bool, Optional[Path], float]:
        ...
        start = time.time()
        try:
            # subprocess.run já lida com buffers. Está OK.
    ...
    def run_parser(self, fsm_file: Path, output_file: Path) -> Tuple[bool, float, float, int]:
        ...
        start = time.time()
        mem_peak_mb = 0.0 # <--- Memória não pode mais ser medida
        
        try:
            proc = subprocess.Popen(
                cmd, cwd=self.scripts_dir,
                stdout=subprocess.DEVNULL, # Joga fora o stdout
                stderr=subprocess.PIPE, 
                text=True
            )
            
            # --- REMOVIDO LOOP DE MEDIÇÃO DE MEMÓRIA ---
            # O loop 'while proc.poll()' foi removido para evitar deadlock.
            
            try:
                # Chamado imediatamente para drenar stdout/stderr
...
def process_path(executor: ExhaustivePipelineExecutor,
    ...
    # Log reduzido para não poluir o output
    if path_idx % 20 == 0:
    ...
    if not path.tables:
        # Este é um caminho "sem tabelas", o estado final é o estado do parser
    ...
    for table_idx, table_name in enumerate(path.tables):
        ...
        table_ok, table_time, table_mem = executor.run_table(
        ...
        current_states = table_output # O próximo input é o output atual

    # O último 'current_states' é o arquivo de estado final para este caminho
...
def main():
    ...
    # Configurações de teste - variando complexidade gradualmente
    test_configs = [
    ...
    for p_states, i_tables, e_tables in test_configs:
        ...
        # Gera apenas a versão Paralela (par)
        try:
            meta_par = generator.generate_program(
                **base_params,
                ingress_logic_type='parallel',
                prog_id_suffix="_par", # Adiciona sufixo para clareza
    ...
    for i, meta in enumerate(all_metadata):
        ...
        log.info(f"Params: {meta['config']}") # Mostra a configuração inteira
        log.info("="*70)
        ...
        for run_num in range(1, NUM_RUNS + 1):
            ...
            # Adicionando os logs de diagnóstico que ajudaram a encontrar o bug
            ...
            if not all_final_states:
                ...
                log.info(f"   [Deparser] Escrevendo arquivo de entrada: {deparser_input}")
    ...
    try:
        ...
        # Gráfico 1: Tempo total vs complexidade (boxplot)
        ...
        # Gráfico 2: Número de caminhos vs complexidade
        ...
        # Gráfico 3: Breakdown de tempo (stacked bar)
        plt.figure(figsize=(12, 7))

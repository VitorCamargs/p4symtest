# run_exhaustive_test.ps1
# Script to execute the EXHAUSTIVE test for P4SymTest
# Tests all possible paths through the pipeline

Write-Host "==================================" -ForegroundColor Blue
Write-Host "P4SymTest - Exhaustive Test (Full Pipeline)" -ForegroundColor Blue
Write-Host "==================================" -ForegroundColor Blue
...
# Required scripts
$LOCAL_SCRIPTS = @(
    ".\exhaustive_test_orchestrator_opt.py",
    ".\synthetic_p4_generator.py"
)

$CONTAINER_RESULTS_DIR = "/app/workspace/exhaustive_test_run"
$LOCAL_RESULTS_DIR = ".\exhaustive_results_$TIMESTAMP"
...
# Em run_exhaustive_test.ps1, Seção 3:

# --- 3. Instalar Dependências ---
Write-Host "`nInstalando dependencias (pandas, matplotlib, z3)..." -ForegroundColor Yellow
# Adiciona z3-solver. psutil não é mais estritamente necessário pelo orquestrador.
docker exec $CONTAINER_NAME pip install pandas numpy matplotlib seaborn z3-solver 2>&1 | Out-Null
...
Write-Host "STARTING EXHAUSTIVE TEST" -ForegroundColor Cyan
...
$SCRIPT_NAME = "exhaustive_test_orchestrator_opt.py"
